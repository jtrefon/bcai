...
use runtime::{
    federated::{
        AggregationStrategy, FederatedConfig, FederatedEngine, ModelMetadata, ModelParameters,
    },
    node::{NodeCapability, UnifiedNode},
    p2p_service::{create_p2p_service, P2PConfig},
};
...
fn demo_p2p_service() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, node) in nodes.into_iter().enumerate() {
        let config = P2PConfig {
            ...
            bootstrap_peers: if i == 0 { vec![] } else { vec![format!("/ip4/127.0.0.1/tcp/4001")] },
            max_peers: 20,
    ...
    for round in 1..=3 {
        ...
        for (i, (node_id, _)) in participants.iter().enumerate() {
            ...
            let local_model = ModelParameters {
                ...
                biases: vec![0.01 + (round as f32 * 0.001), 0.02 + (round as f32 * 0.001)],
                layer_sizes: vec![10, 5, 2],
                metadata: ModelMetadata {
                    model_id: "simple_classifier".to_string(),
            ...
            println!(
                "   📤 {} uploaded local model (acc: {:.3})",
                node_id,
                0.2 + (round as f32 * 0.2) + (i as f32 * 0.05)
            );
        }

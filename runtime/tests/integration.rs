...
fn simplified_distributed_workflow() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let capability =
        NodeCapability { cpus: 4, gpus: 1, gpu_memory_gb: 8, available_stake: 200, reputation: 0 };
    ...
    println!(
        "📊 Stats: Active Jobs: {}, Worker Balance: {}",
...
fn message_routing_test() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let capability =
        NodeCapability { cpus: 4, gpus: 1, gpu_memory_gb: 8, available_stake: 100, reputation: 0 };
    ...
    println!(
        "📊 Network Stats: Node1 peers: {}, Node2 peers: {}",
...
fn capability_matching_and_filtering() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let poster_capability =
        NodeCapability { cpus: 8, gpus: 2, gpu_memory_gb: 32, available_stake: 0, reputation: 0 };
    let poster_node = UnifiedNode::new("poster".to_string(), poster_capability, 2000);
    let mut poster_coordinator = NetworkCoordinator::new(poster_node);

    // Create a low-capability worker
    let low_capability =
        NodeCapability { cpus: 2, gpus: 0, gpu_memory_gb: 4, available_stake: 0, reputation: 0 };
    ...
    let job_requirements =
        NodeCapability { cpus: 6, gpus: 2, gpu_memory_gb: 16, available_stake: 400, reputation: 0 };
    ...
    // Test responses
    let low_worker_responses =
        low_worker_coordinator.handle_message(job_message.clone(), "poster")?;
    ...
    if let NetworkMessage::JobVolunteer { job_id, node_id, capability: _ } =
        &high_worker_responses[0]
    {
        assert_eq!(*job_id, 1);

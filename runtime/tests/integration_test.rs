...
use runtime::{
    ...
    VmConfig, TensorId, DataType, MLInstruction, PythonConstraints,
};
...
async fn test_full_system_integration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if total_duration > Duration::from_secs(config.test_timeout_seconds) {
        return Err(format!("Integration tests took too long: {:?} > {}s", 
                          total_duration, config.test_timeout_seconds).into());
    }
...
async fn test_basic_vm_functionality() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = vm.execute_instruction(MLInstruction::TensorCreate {
    ...
    let result = vm.execute_instruction(MLInstruction::TensorAdd {
        a: TensorId(1),
        b: TensorId(2),
        output: TensorId(3),
    }).await?;
...
async fn test_ml_workload_processing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let result = vm.execute_instruction(MLInstruction::TensorMatMul {
        a: input_tensor,
        b: weight_tensor,
...
"#;
    ...
    let result = vm.execute_instruction(MLInstruction::PythonExecute {
...
async fn test_performance_requirements() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if creation_throughput < MIN_TENSOR_CREATION_THROUGHPUT {
        return Err(format!("Tensor creation throughput too low: {:.2} < {:.2} ops/sec", 
                          creation_throughput, MIN_TENSOR_CREATION_THROUGHPUT).into());
    }
    ...
    for i in 0..num_matmuls {
        ...
        vm.execute_instruction(MLInstruction::TensorMatMul {
            a: a_id,
            b: b_id,
            output: output_id,
        }).await?;
    ...
    if matmul_throughput < MIN_MATMUL_THROUGHPUT {
        return Err(format!("Matrix multiplication throughput too low: {:.2} < {:.2} ops/sec", 
                          matmul_throughput, MIN_MATMUL_THROUGHPUT).into());
    }
...
async fn test_security_requirements() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if block_rate < MIN_BLOCK_RATE {
        return Err(format!("Security block rate too low: {:.2} < {:.2}", 
                          block_rate, MIN_BLOCK_RATE).into());
    }
...
async fn test_resource_management() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Clean up tensors
    for i in 0..10 {
        vm.execute_instruction(MLInstruction::TensorDestroy {
            tensor_id: TensorId(i),
        }).await?;
    }
    ...
    if memory_reduction_ratio < 0.5 {
        return Err(format!("Insufficient memory cleanup: {:.1}% reduction", 
                          memory_reduction_ratio * 100.0).into());
    }
...
async fn test_concurrent_operations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if concurrent_duration > Duration::from_secs(5) {
        return Err(format!("Concurrent operations took too long: {:?}", concurrent_duration).into());
    }

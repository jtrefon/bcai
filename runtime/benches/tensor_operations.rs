...
use runtime::{
    tensor_ops::{TensorManager, Tensor},
    TensorId, DataType, VmError,
};
...
fn bench_tensor_arithmetic(c: &mut Criterion) {
    ...
    for (i, shape) in shapes.iter().enumerate() {
        ...
        manager.create_tensor(TensorId(i as u64 * 2 + 1), shape.clone(), DataType::Float32).unwrap();
...
fn bench_activations(c: &mut Criterion) {
    ...
    for (i, shape) in shapes.iter().enumerate() {
        ...
        group.bench_with_input(
            ...
            |b, &i| {
                b.iter(|| {
                    let result = manager.tensor_relu(
                        TensorId(i as u64),
                        TensorId(300 + i as u64),
                    );
                    black_box(result)
                });
            },
    ...
    if let Ok(gpu_device) = Device::new_cuda(0) {
        for shape in shapes {
            ...
            group.bench_with_input(
                ...
                |b, shape| {
                    b.iter(|| {
                        let tensor_a = Tensor::new(TensorId(0), shape.clone(), DataType::Float32, &cpu_device).unwrap();
                        let tensor_b = Tensor::new(TensorId(1), shape.clone(), DataType::Float32, &cpu_device).unwrap();
            ...
            group.bench_with_input(
                ...
                |b, shape| {
                    b.iter(|| {
                        let tensor_a = Tensor::new(TensorId(0), shape.clone(), DataType::Float32, &gpu_device).unwrap();
                        let tensor_b = Tensor::new(TensorId(1), shape.clone(), DataType::Float32, &gpu_device).unwrap();
...
fn bench_data_types(c: &mut Criterion) {
    ...
    for (i, dtype) in data_types.iter().enumerate() {
        group.bench_with_input(
            ...
            |b, dtype| {
                b.iter(|| {
                    let result = manager.create_tensor(
                        TensorId(i as u64),
                        shape.clone(),
                        dtype.clone(),
                    );

...
impl NeuralNetwork {
    ...
    pub fn train_epoch(&mut self, data: &TrainingData) -> f32 {
        ...
        for (input, target) in data.inputs.iter().zip(data.targets.iter()) {
            ...
            let loss: f32 =
                output.iter().zip(target.iter()).map(|(o, t)| (o - t).powi(2)).sum::<f32>()
                    / output.len() as f32;
    ...
    fn backward(&mut self, input: &[f32], target: &[f32], output: &[f32]) {
        ...
        // Output layer error
        let output_error: Vec<f32> = output
            .iter()
            .zip(target.iter())
        ...
        for layer_idx in (0..self.layers.len() - 1).rev() {
            ...
            for (j, layer_error_item) in layer_error.iter_mut().enumerate() {
                for (k, current_error_item) in current_error.iter().enumerate() {
                    *layer_error_item += current_error_item * next_layer.weights[k][j];
                }
            }
    ...
    pub fn train(&mut self, data: &TrainingData, epochs: u32) -> Vec<TrainingMetrics> {
        ...
        for epoch in 0..epochs {
            ...
            metrics.push(TrainingMetrics { epoch: epoch + 1, loss, accuracy, training_time_ms });
